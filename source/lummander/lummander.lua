...
local Command = require "source.lummander.command"
local chalk = require "source.chalk"
local ftable = require("source.f.table")
local fstring = require("source.f.string")
local Pcall = require"source.lummander.pcall"
local Parsed = require "source.lummander.parsed"
local ThemeColor = require"source.lummander.themecolor"
...
function Lummander:apply_theme(theme)
    ...
    theme = theme or require"source.lummander.themes.default"

    if(type(theme) == "string")then
        self.pcall(function()
            theme = require("lummander.themes." .. theme)
        end):fail(function(err)
            theme = require"source.lummander.themes.default"
        end)
    end
...
function Lummander:help(ignore_flag)
    ...
    print(self.theme.cli.title.color(self.title .. " (v" .. self.version .. ")".. ((#self.author > 0 and " by " .. self.author) or "") .. "\n") ..self.theme.cli.category.color.bold("Usage: ")
    .. self.tag .. " <command> [options] ")
    if(#self.description > 0)then print(self.theme.cli.category.color.bold("Description: ") .. self.description) end
    if(self.default_action and self.default_action_command) then print(self.theme.cli.category.color.bold("Default command: ")..self.default_action_command.name.. ((#self.default_action_command.description > 0 and " -> " ..self.default_action_command.description .. "\n") or "")) end
    self.theme.cli.category(chalk.bold("Commands:"))
    ftable.for_each(self.commands, function(cmd)
        cmd:usage()
    end)
    print()
    print(self.theme.cli.category.color.bold("Use: ") .. self.theme.command.definition.color.bold(self.tag .. " help <command>") .. " to get more info about that command")
end
